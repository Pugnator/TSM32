...
void eeprom_test()
{
  PrintF("EEPROM init %s\r\n", ee_init() ? "OK" : "FAILED");  
  char* test_string = "Here goes the test string.";
  ee_write(0, strlen(test_string) + 1, (uint8_t *)test_string);
  char result[64] = {0};
  ee_read(0, strlen(test_string) + 1, (uint8_t*) result);
  PrintF("EEPROM write/read result - %s, [%s]\r\n", !memcmp(result, test_string, strlen(test_string) + 1)? "OK" : "FAILED", result);
}