...
| [`Core/Src/tsm.cc`](../Core/Src/tsm.cc) | Application: presence heartbeat, security responder, auto‑DTC FSM |
...
The RPM/KPH signals feed the J1850 engine‑state starter lock
([`engine_state.cc`](../Core/Src/engine_state.cc)); the DTC/security state drives
the auto‑DTC FSM and lamp behaviour.
...
- **Blind‑retry safety net** — a `LostArbitration` triggers one retry with the
  arbitration monitor disabled, so a miscalibrated monitor can never permanently
  kill the TX path (a field build once tripped on 100 % of transmissions).
...
flowchart TD
    ...
    subgraph DTC["Auto‑DTC FSM (10 s cycle)"]
      D1["query ECM 0x10,<br/>IPC 0x60/0x61 (19 52 FF 00)"]
      D1 --> D2["parse 0x59 responses<br/>(raw + status byte logged)"]
      D2 --> D3{"password DTC?<br/>SIL held >1 s?"}
      D3 -->|yes| D4["clear ECM 0x14 /<br/>IPC 0x14 (rate‑limited)"]
      D3 -->|no| D5["cooldown 10 s"]
      D4 --> D5
      D5 --> D1
    end
...
- **Auto‑DTC FSM** — cycles every 10 s: query ECM/IPC for stored codes, decode
  the `0x59` responses (logging raw bytes + the KWP status byte that
  distinguishes current vs historic), and *continuously* clear a re‑appearing
  password DTC or a code behind a held SIL — no one‑shot latch, rate‑limited to
  protect module flash.
